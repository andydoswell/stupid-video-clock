...
boolean modeChangeFlag = true ; // true if display mode has changed
...
void loop() {
  ...
  if (pserial.available()) { // Read seconds from IC1
    secs = ((char)pserial.read());
  }

  if (oldsecs != secs) { // update time
    oldsecs = secs;
    modeSecs++;
    if (!secs) {
      mins++;
  ...
  if (mins == 59 && secs == 59 && !chimeFlag ) { // if the clock is about to chime, ensure mode 0 is active.
    mode = 0;
    modeSecs = 0;
    TV.clear_screen();
    drawClockFace();
...
void clockSet () {
  ...
  if (!digitalRead (setPin)) { //! in here before final compile
    ...
    TV.delay (10);
    digitalWrite(runTimer, HIGH);// start it
    return;
  }
  clockSet();
}
...
void getTemp () { // read the temperature
  thermRaw = 0;
  for (int i = 0; i <= 99; i++) {
...
void matrix () {
 ...
 TV.set_cursor (random(120),random(88));
 TV.write (random(32,96));
    timeX = 40;
    timeY = 75;
    tempX = 44;
    tempY = 87;
